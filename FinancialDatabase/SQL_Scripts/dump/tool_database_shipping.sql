...
CREATE TABLE `shipping` (
  ...
  `ItemID_shipping` int unsigned NOT NULL,
  `Notes_shipping` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`SHIPPING_ID`),
  UNIQUE KEY `ShippingID_UNIQUE` (`SHIPPING_ID`),
  KEY `ItemID_idx` (`ItemID_shipping`),
  CONSTRAINT `ItemIDShipping` FOREIGN KEY (`ItemID_shipping`) REFERENCES `item` (`ITEM_ID`) ON DELETE CASCADE ON UPDATE CASCADE
